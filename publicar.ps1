...
# y ahi dentro no puede haber nada que abra el panel. Quien entra en la
...
if ($Clave -eq "") {
    if (-not (Test-Path "clave.txt")) {
        ...
        Write-Host "       Escribe la clave del panel en clave.txt, o lanza:" -ForegroundColor DarkGray
...
Write-Host ""
Write-Host "Publicado en https://drodridavid.github.io/proyectos-uja/" -ForegroundColor Green
Write-Host ""
Write-Host "Si has cambiado la clave, borra clave-panel.txt de la carpeta de Drive" -ForegroundColor Yellow
Write-Host "'Cuaderno Proyectos (UJA)' y vuelve a entrar para dejar la nueva." -ForegroundColor Yellow
Write-Host ""
